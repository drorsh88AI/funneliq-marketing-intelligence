...
## חבילה 5 — נשירה במשפך, calls_to_closed (אוכלוסייה: 3,500 / 3,318, dataset description)
...
התפלגות `calls_to_closed` בתוך אוכלוסיית `closed>0` (3,318 רשומות):

![calls_to_closed distribution](calls_to_closed_distribution.svg)

תרשים עמודות: התפלגות calls_to_closed בפועל בתוך אוכלוסיית closed>0 -- לכל ערך calls_to_closed שנצפה בקובץ (ציר x), ספירת הרשומות עם אותו ערך בדיוק (ציר y), בלי חלוקה ל-bins מומצאים. סך התדירויות שווה population_n=3318. calls_to_closed מתועד ב-SPEC.md כממוצע ברמת רשומה, לא כהיסטוריית שיחות לעסקה בודדת. כל הערכים לקוחים ישירות מ-results['calls_to_closed']['frequency_by_calls_to_closed'], בלי חישוב חוזר מה-CSV.

מתוך 3,318 רשומות `closed>0`, החציון הוא 3, השכיח הוא
2 והממוצע 3.706. ב-1,595 מהן (48.07%) `calls_to_closed`
הממוצע הוא 4 ומעלה. בתת-האוכלוסייה `closed=1` (510 רשומות),
458 מהן (89.80%) מציגות `calls_to_closed >= 4`.
...
## מסקנת P5 — תיאורית, לא סיבתית (אוכלוסייה: 3,500 / 3,318, dataset description)

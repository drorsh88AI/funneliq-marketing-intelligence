...
- `B45`/`B50`/`B28`/`B36`/`B41`/`B52` ⇒ `done` על ראיית **CP4** (חשיפה
  בדשבורד החי), כולל שער `P12-D9` ל-`B28`/`B41`/`B50` (⛔ **תוקן
  2026-09-23, נמצא ב-CP8:** הנוסח הקודם כאן כתב "ל-`B45`/`B50`" —
  טעות הקלדה; `P12-D9` מגדירה שלושה צרכנים — P2/P4S/Budget — ו-⛔ לא
  `B45`. ר' `PHASE12.md` §ו לתיקון המלא).
...
- `B59` ⇒ נשארת `planned` גם אחרי מסע מוצלח; רכיב התיעוד (`README.md`)
  בבעלות פאזה 13.
- ⚠ ל-`B28`/`B41`/`B50` (⛔ לא `B45` — ר' תיקון למעלה) חל בנוסף שער
  `P12-D9`: הנוסח הקנוני של `DESIGN.md` §6.1 חייב להופיע במסלול
  **הבריא** עם הערכים החיים, בראיה חיובית — ⛔ היעדר נוסחי §6.1ג אינו
  ראיה מספקת. `B36`/`B45`/`B52` נסגרו ב-CP8 על ראיה חיה אחרת, ⛔ ללא
  שער D9 ייעודי (ר' שורותיהן ב-`REQUIREMENTS.md` לפירוט).
...
| **B21** | §05.1 | "and conversion rate (closed / num_leads) across budget tiers — Low (≤1500), Mid (2000–5000), High (>5000)." | שיעור המרה לפי שלושת הטיירים | 5 | 9, 10, 11, 10A, 12A | 12 | done | `docs/findings.json.budget_tiers` — Low **0.04524** (4.5%, n=780) · Mid **0.08220** (8.2%, n=1,717) · High **0.05433** (5.4%, n=1,003) · gap n=0 |
...
| **B24** | §05.1 | "Which budget tier converts best — and does that surprise you?" | תשובה כתובה | 5 | 10, 11, 13, 10A, 12A | 12 | gap | `FINDINGS.md` מוכיח ש־Mid מוביל עם 8.2%; ההסבר האם ומדוע התוצאה מפתיעה חסר |
...
| **B28** | §05.2 | "Expose the prediction through your app." | חשיפת חיזוי ה-LTV באפליקציה | 9 | 10, 11, 10A, 12A | 12 | done | ראיית ביניים נשמרת: `/api/predict/ltv` חי ומאומת מול הארטיפקט ו־JWT (PHASE9 ק' 75). 10A/CP5 נעל זרימת קלט עצמאית/דוגמה; CP6 מיפה את התוצאה ל־`LtvPrediction.point_estimate/lower_bound/upper_bound` ואת OOD ל־`in_training_domain/warnings[]`. **המימוש בוצע בפאזה 11** (CP4/CP5, פאנל P2 בטופס המשותף, כולל OOD פר-פאנל וכשל חלקי; מקרי הפרכה 14/15). **ראיית קבלה חיה נוספה (CP8, 2026-09-23):** `live/test_03_capabilities.py` (11 בדיקות, 110.56s), שער `P12-D9` (ראיה חיובית) על `predict/ltv` — תגובת הרשת המדויקת נתפסה, ארבע שכבות D9 נבנו ממנה בפייתון לפי `predict.js` file:line, והושוו בשוויון מלא מול ה-DOM החי; `model_version` אומת בהסכמה משולשת מול `meta`/`business_facts.json` (`P12-D14`). ר' `ROADMAP.html` checkpoint 4 |
...
| **B36** | §02 | "Each answer must be reachable through your deployed, signed-in application — not just printed once in a notebook cell." | חיזוי ה-upsell נגיש דרך האפליקציה | 9 | 10, 11, 10A, 12A | 12 | done | ראיית ביניים נשמרת: `/api/predict/upsell` חי ומאומת עם JWT (PHASE9 ק' 75). 10A/CP6 מיפה את הפלט העסקי ל־`event_probability/base_rate/propensity_band` והוציא Accuracy/Precision/Recall שאינם בחוזה הדפדפן; **החשיפה בוצעה בפאזה 11** (CP5, פאנל P3 עם `summary-recommendation`, תג כיול ו-`model-details` מתקפל). **ראיית קבלה חיה נוספה (CP8, 2026-09-23):** `live/test_03_capabilities.py`, סכמת `PropensityPrediction` נבדקה מול הנתיב החי דרך `app.schemas` האמיתי, ו-`model_version` אומת בהסכמה משולשת מול `meta`/`business_facts.json` (`P12-D14`); ⛔ ללא שער D9 4-שכבות ייעודי (P3 אינו אחד משלושת צרכני `business_facts.json` לפי `P12-D9`). ⚠ **דיוק (CP8, סבב שני):** בדיקת סכמת API לבדה אינה מוכיחה "reachable through your application" — נוספה `live/test_05_edge_cases.py` (CP6, `test_path_failure_shows_error_and_recovers_on_retry`): באותה שליחת טופס אחת שבה `predict/ltv` הוזרק לכשל, P3 (לא הוזרק) הציג **תוצאה אמיתית בפועל על המסך**, מה-DOM, אחרי שליחה דרך הטופס המשותף — ⛔ לא רק תגובת API שנבדקה בנפרד מהממשק. ר' `ROADMAP.html` checkpoints 4/6 |
...
| **B41** | §05.4 | "Then build a scoring pipeline: given a new customer's early funnel data, output a 0–100 likelihood of becoming a super customer, served by your app." | ציון 0–100 ללקוח-על, מוגש באפליקציה | 8A | 9, 10, 11, 10A, 12A | 12 | done | CP9 פרס CatBoost ייעודי ליעד P4S עם ארבעת הקלטים: `P4S-catboost-20260912-1c70ca8`; ה־API מחזיר אותו באותה סכמה. Logistic נשמר להשוואה. **הציון 0–100 נחשף בפאזה 11** (CP6, מסך P4S עצמאי: ארבעה שדות, `Math.round(p*100)`, כרטיס הקשר עסקי, ⛔ בלי טווח/צביעה/`uncalibrated`; מקרי הפרכה 18/19 + `docs/design/states/p4s-*.jpg`). **ראיית קבלה חיה נוספה (CP8, 2026-09-23):** `live/test_03_capabilities.py`, שער `P12-D9` (ראיה חיובית) על `predict/super-customer` — תגובת הרשת המדויקת נתפסה, ארבע שכבות D9 נבנו ממנה בפייתון לפי `super-customer.js` file:line, והושוו בשוויון מלא מול ה-DOM החי; `model_version` אומת בהסכמה משולשת (`P12-D14`). נוסח `B41` עצמו דורש רק "ציון 0–100 תקף שמוגש" — ⛔ לא ששונה בין דוגמאות — ונסגר על בסיס זה. ⚠ **קשור, ⛔ לא חוסם:** `docs/acceptance/EVIDENCE.md` §ה מתעד ממצא נפרד (חלקית מאומת) של ציון P4S נמוך וחוזר על עצמו בכמה קלטים שנבדק ביוזמת בוחן חיצוני; זה נשאר תנאי מקדים פתוח ל-`B55b` (ר' שם), ⛔ אינו פוסל את סגירת `B41` הצרה. ר' `ROADMAP.html` checkpoint 4 |
| **B42** | §05.4 | "Profile the super customers (referred = Yes, upsell = 1, long tenure): what share of total profit do they represent, and what's their average acquisition cost?" | פרופיל: אחוז רווח + CAC | 6 | 10, 11, 13, 10A, 12A | 12 | done | `metrics.json.super_customer_profile` — n=529, 33.61% מהרווח, CAC 990.7 מול 1,437.5 (חיסכון 31.1%); CP9 הפיק העתק מצומצם וגרסאי ב־`app/static/business_facts.json` |
...
| # | מקור | ציטוט מהבריף | חובה | בעלים | תורמים | מאמת | סטטוס | ראיה |
|---|---|---|---|---|---|---|---|---|
| **B44** | §05.5 | "the dropout rate at each follow-up stage (followup_1→followup_5), visualized" | נשירה בכל שלב, ויזואלית | 5 | 9, 10, 11, 10A, 12A | 12 | done | `findings.json.funnel_dropoff` — 21.7/25.7/18.6/10.4/29.2%; `funnel_dropoff.svg` |
| **B45** | §05.5 | "plus a data-driven recommendation surfaced in the dashboard" | המלצה מוצגת בדשבורד | 11 | 5, 9, 10, 10A, 12A | 12 | done | טקסט מ־`SPEC.md` §הכרעת CP4-D; CP5 נעל פעולה/מגבלה גלויות, ו־CP6 נעל שהמלצה משולבת תלויה בשני חלקי `FollowupResponse` התקינים וללא fallback קבוע. החשיפה בוצעה בפאזה 11 (CP8, `app/static/js/screens/followup.js`): פסקת המלצת CP4-D מוצגת מילה במילה, ורק כששני חלקי `FollowupResponse` תקינים — כשל חלקי מסתיר את ההמלצה המשולבת ⛔ בלי מספר שמור חלופי. ראיה: מקרה הפרכה 23 + `docs/design/states/followup-error.jpg`. ✅ **ראיית success שהייתה חסרה נוספה (11A checkpoint 10, 22.09.2026, ⛔ לא ראיה סופית):** `docs/design/states/followup-success.jpg` (`P11A-D11`, checkpoint 8, commit `a3455d0`) — רינדור אמיתי מציג את שני הבלוקים (נשירה + `calls_to_closed`) מאוכלסים ואת פסקת המלצת CP4-D המשולבת בפועל; ⛔ פסקת ההמלצה עצמה לא שונתה ב-11A (לא נפסלה, ר' לעיל). **ראיית קבלה חיה נוספה (CP8, 2026-09-23), ⚠ ללא שער D9 ייעודי — Follow-up אינו אחד משלושת צרכני `business_facts.json` לפי `P12-D9`:** שלוש ראיות משלימות. ⚠ **דיוק (CP8, סבב שני):** ההמלצה המשולבת (פסקת CP4-D) היא **נוסח קבוע** שנקבע מראש מניתוח הנתונים המקורי (לא מחושב מחדש מתוך התגובה החיה), ומוצג **כאשר הנתונים זמינים** — כלומר הראיות שלהלן מוכיחות זמינות/הצגה, ⛔ לא חישוב חי מחדש: (1) `live/test_03_capabilities.py` (CP4) אימת את סכמת `FollowupResponse` מול הנתיב החי, כלומר שהנתונים הדרושים לתנאי ההצגה זמינים בפועל; (2) `live/test_05_edge_cases.py` (CP6, `test_partial_followup_failure_shows_healthy_half_only`) הוכיח בערך אמיתי מהשרת (לא הוזרק) ש-`stages` מוצג נכון כשהוא בריא, לצד בידוד כשל מוזרק בחלק השני — כלומר תנאי ההצגה (שני החלקים תקינים) נבחן מול נתונים אמיתיים; (3) בוחן חיצוני אמיתי (CP7, `P12-D10`) צפה בפועל בפסקת ההמלצה הקבועה במסך "מעקב שיחות" מול הפריסה החיה בהתחברות אמיתית וכתב ביקורת תוכן מפורטת עליה (`docs/acceptance/EVIDENCE.md` §ד) — ראיה שההמלצה אכן הוצגה בדשבורד, גם אם קריאותה הייתה קשה. ר' `ROADMAP.html` checkpoints 4/6/7 |
| **B46a** | §05.5 | "At which stage does dropout behave unexpectedly?" | תשובה כתובה | 5 | 10, 11, 13, 10A, 12A | 12 | done | `funnel_dropoff.followup_4 = 0.10372` — הנמוך ביותר |
| **B46b** | §05.5 | "For deals that eventually closed, how many follow-ups did they typically take?" | תשובה כתובה | 5 | 10, 11, 13, 10A, 12A | 12 | done | `SPEC.md` §הכרעת CP4-D ו־`docs/findings.json.calls_to_closed`: באוכלוסיית `closed>0` ‏(3,318) החציון 3, השכיח 2, הממוצע 3.706 ו־1,595 (48.07%) עם ממוצע 4+; CP9 יישר את ה־API לאותה אוכלוסייה עם עימוד וספירה עצמאית |
| **B46c** | §05.5 | "Should Northbound change its follow-up policy — yes or no, and why?" | תשובה כתובה | 5 | 10, 11, 13, 10A, 12A | 12 | done | `SPEC.md` §הכרעת CP4-D ו־`FINDINGS.md` §מסקנת P5 — לא לעצירה אוטומטית; להמשיך מבוקר ולמדוד עלות ושיעור סגירה שולי |
...
| **B52** | §05.6 | "Expose the simulator in your app." | חשיפת הסימולטור | 9 | 10, 11, 10A, 12A | 12 | done | `/api/simulate/budget` מחזיר את `P6_simulation.json` המתוקן כלוקאפ GET ללא body; חוזה ה־API עבר בבדיקות. **הממשק נבנה בפאזה 11** (CP7, `screens/budget.js` — GET בלבד, ⛔ אפס קלט משתמש, גרף + fallback טבלאי; מקרי הפרכה 21/24). **ראיית קבלה חיה נוספה (CP8, 2026-09-23):** `live/test_03_capabilities.py`, שער `P12-D9` (ראיה חיובית) על `simulate/budget` — ר' `B50` לפרטי אותה ראיה (אותו נתיב/מסך). ר' `ROADMAP.html` checkpoint 4 |
| **B53a** | §05.6 | "Based on what your model learned about diminishing returns, does concentrating or spreading spend win?" | תשובה כתובה | 13 | 6, 10, 11, 10A, 12A | 12 | gap | CP9: `100x500` מדורגת ראשונה (789,594) ו־`25x2000` שנייה (530,953), אך הטווחים חופפים וה־backtest ברמת 500 רחוק מאוד מהתחזית. אין הוכחה שפיזור קיצוני מנצח; נשארה הטמעה ב־REPORT |
| **B53b** | §05.6 | "What would you tell the founder to do next month?" | תשובה כתובה | 13 | 6, 10, 11, 10A, 12A | 12 | gap | ההמלצה המתוקנת: לא להעביר 50,000 ש״ח ל־100 קמפיינים על סמך המודל; אם בוחנים אחת מארבע החלופות, לבצע פיילוט מוגבל בדפוס `25x2000` ולמדוד רווח מצטבר באותו אופק. נשארה הטמעה ב־REPORT |
...
| **B55b** | §06 | "A live application URL on Railway — a real link you can share that actually answers the founder's questions." | האפליקציה הפרוסה **עונה בפועל** לשאלות המייסדת | 12 | 9, 10, 11, 13, 10A, 12A | 12 | planned | ⚠ ה-URL חי (B55a). 10A/CP5 מיפה את חמש התשובות לכרטיסי יכולת גנריים בלי לחשוף את ניסוח המשימה. ⚠ **עודכן 22.09.2026 — הנוסח הקודם ("התוכן והמימוש החי טרם קיימים") התיישן:** פאזה 11 מימשה את חמשת המסכים ופאזה 11A יישרה אותם לחוזה התכן, ובבדיקת השער נמצא שהפריסה מגישה את build 11A בפועל (8/8 הנכסים ש-11A שינתה — זהים ל-blob שלהם ב-`HEAD`). מה שחסר אינו המימוש אלא **ראיית הקבלה**: אף אחד עדיין לא קיבל מהגרסה החיה, בהתחברות אמיתית, תשובה מנוסחת לכל חמש השאלות. סגירה ⇐ CP4 + CP7 בפאזה 12, וכל **חמש** השאלות (⛔ לא ארבע, ⛔ לא "רובן"). **עדכון CP8 (2026-09-23), ⛔ עדיין לא `done`:** CP4 סיפק ראיה חיה לחמש תגובות המודל (`B28`/`B36`/`B41`/`B45`/`B50`), ו-CP7 סיפק בוחן חיצוני אמיתי שהשלים את כל שבע רגלי `B59` וקרא בפועל את חמשת בלוקי ההמלצה (`docs/acceptance/EVIDENCE.md`). ⛔ **זה עדיין אינו סגירה**: הבוחן דיווח קושי עקבי **להבין** את מה שקרא (§ד), וממצא נפרד (§ה, חלקית מאומת) על ציון P4S נמוך/חוזר על עצמו לא נבדק עד תום. "actually answers" דורש גם הבנה, ⛔ לא רק קריאה. הכרעת המשתמש (2026-09-23): ממשיכים כעת בתוכנית; `B55b` נשארת `planned` וממצאי §ד/§ה נשארים פתוחים לטיפול בהמשך, בלי שיוך אוטומטי לפאזה 13 או לכל פאזה אחרת |
| **B56** | §06 | "A login-gated dashboard — a real Supabase Auth sign-in screen, not an open page, so the app behaves like the internal tool it's meant to be." | דשבורד מאחורי לוגין | 11 | 4, 10, 10A, 12A | 12 | done | פאזה 11 (CP1): מסך התחברות Supabase Auth אמיתי (`signInWithPassword` דרך הלקוח הרשמי; ⛔ `anon/publishable key` בלבד בדפדפן), ⛔ לא דף פתוח — `authenticated-shell` מוצג רק אחרי `200` מ-`/api/me`, ו-`403` (ארגון שאינו `northbound`) מציג `forbidden-notice` ללא נתונים כלל. ראיה: מקרי הפרכה 6/9ב/12 + `docs/design/states/login-signin-error.jpg`/`bootstrap-403.jpg`; הפריסה החיה אומתה כמגישה את ה-SPA (שער 11→12, 18.09.2026; אומת מחדש 22.09.2026 אחרי 11A — 8/8 הנכסים ש-11A שינתה מוגשים זהים ל-blob שלהם ב-`HEAD`). **ראיית קבלה חיה נוספה (CP8, 2026-09-23):** `live/test_01_permissions.py`, הרצת המשתמש — 5/5 עברו. התחברות אמיתית מול Supabase Auth הוכחה (`demo-northbound` ⇒ `authenticated-shell`), `demo-noorg` ⇒ 403 ברמת הרשת על `/api/me` ואפס תגובות 2xx על תשעת המשאבים העסקיים הנעקבים, ללא טוקן ⇒ 401. ר' `ROADMAP.html` checkpoint 2 |

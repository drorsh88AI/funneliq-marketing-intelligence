...
| `clear-form-action` | `נקה טופס`, P4S בלבד — מאפס ארבעה שדות, דוגמה, אישור הקשר ותוצאה, ומאפס את `input-summary` ל-`תרחיש עצמאי · 0/4`. **דורש אישור כשקיים ערך, דוגמה, עריכה או תוצאה**; ישיר רק בטופס ריק לחלוטין. ביטול האישור ⛔ אינו משנה מצב. הרחבה לטופס המשותף היא הכרעה נפרדת (`IA.md` §3א.3.1) | — |
